...
**痛点**
- 默认分类 `DEFAULT_CATEGORY_ID=1L` 是门面接口常量（[ConstantConfigCenter.java](file:///d:/study/常量配置中心/Constant-Config-Center/src/main/java/com/constantconfig/center/api/ConstantConfigCenter.java#L39)）；分页默认 `size=10` 在 `ConfigPageQuery`、`CategoryPageQuery`、`Impl.normalizeSize` 三处重复；`version` 初始 `0`、`path` 形如 `/1` 散落 SQL。
...
**改动方向**：抽纯函数式工具 `CategoryTreeAssembler.assemble(List<CategoryView>)`，可单测；门面只调用。
...
**改动方向**
- 新增 `SaveConfigCommand` / `SaveCategoryCommand`：写操作入参，不含 `version`、`createTime/updateTime`、`children`，`valueObject` 归并进 `value` 或命令自身。
- 新增 `ConfigView` / `CategoryView`：读操作返回（配置视图不含内部字段；分类视图含 `children` 但剥掉存储字段）。
